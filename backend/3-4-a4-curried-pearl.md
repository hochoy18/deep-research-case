...
用户要把 `backend/docs/html/` 下的 ZhiPo DeepResearch 多智能体深度研究后端写进个人简历，目标岗位是 **AI 应用 / Agent 工程师**，项目在简历中定位为 **多个项目经历之一（紧凑展示）**，叙事风格采用 **STAR 法则**，差异化亮点以 **多智能体编排 + HITL 断点续传为主、KB 知识库为次、其余作补充**。
...
1. **ZhiPo DeepResearch · 多智能体深度研究后端**（最清晰，含产品名）
2. **DeepResearch Agent 后端 · 多智能体编排平台**（强调 Agent 平台属性）
3. **Deep Research 多智能体研究系统**（最简短，去品牌化）

**推荐 #1**：保留 ZhiPo 品牌名体现产品意识；副标题点明"多智能体 + 深度研究"两个关键词，匹配 Agent 工程师岗位 JD 高频词。
...
### 项目标题行
> **ZhiPo DeepResearch · 多智能体深度研究后端** | 独立开发者 | 2025.05 – 至今
